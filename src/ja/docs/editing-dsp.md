...
`voiced`（非ゼロ＝有声）と `voicedProb` は任意で、省略するとすべてのフレームを有声として扱います。`hopLength` は `f0Hz` を生成した値に合わせ、フレーム `i` がサンプル `i * hopLength` に対応するようにしてください。
...
- `effects.modulation.ensemble` — BBD 方式のストリングマシン・アンサンブル。薄いソースを広がりのある、コーラスのかかったパッドに厚くします。
