...
- 内部のオーバーサンプラーとトゥルーピーク段はオーバーサンプリング係数として 1〜16 の 2 のべき乗（1, 2, 4, 8, 16）を受け付けます（ライブメーターも同じ係数）。CPU と引き換えにインターサンプルピークの精度を上げます。
- UI 向けには表示用に間引いたメーターのバリアントがあります。`meteringVectorscopeDecimated(...)` と `meteringPhaseScopeDecimated(...)` は点列を最大 `maxPoints` 点に間引きます。加えて `meteringSpectrumFrame(...)` は、スペクトラムアナライザーのスナップショット向けの単一フレーム（時間平均なし）のスペクトラムリーダーです。
...
ミキサーシーンのインサートはマスタリングインサートと同じファクトリを使いますが、有効なインサート集合は `masteringProcessorNames()` より少し広いです。インサートの全リストは実行時に `masteringInsertNames()` で取得でき、各インサートが受け付ける構築用パラメータキーは `masteringInsertParamNames(name)`（Python は `mastering_insert_param_names(name)`）で列挙できます。バンド／サブバンド型のプロセッサはインデックス付きの `band{i}.*` キーを列挙し、未知の名前には空配列を返します。リアルタイムオートメーション可能なサブセットが必要な場合は `masteringInsertParamInfo(name)`（Python は `mastering_insert_param_info(name)`）を使います。各パラメータの JSON キー、数値のオートメーション id、リアルタイム安全フラグを返します。一覧外のキーはプロセッサに無視され、そのキーを含むシーンを読み込むと [`Mixer.sceneWarnings()`](./mixing-scene-json.md) が報告します。上のソロプロセッサに加え、クリエイティブ FX 有効ビルドではリバーブやモジュレーションのインサート ID も使えます。

ピッカー UI では `masteringProcessorCatalog()`（Python は `mastering_processor_catalog()`）を使うと、`kind`、`realtimeInsertable`、`stereoOnly`、`channelPolicy` を持つ機械処理しやすいエントリを取得できます。これにより、オフライン専用、ペア処理、ステレオ専用、サラウンドバスでのラップ方法をプロセッサ ID のハードコードなしに絞り込めます。
