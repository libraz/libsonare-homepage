...
ミキサーシーンのインサートはマスタリングインサートと同じファクトリを使いますが、有効なインサート集合は `masteringProcessorNames()` より少し広いです。インサートの全リストは実行時に `masteringInsertNames()` で取得でき、各インサートが受け付けるパラメータキーは `masteringInsertParamNames(name)`(Python は `mastering_insert_param_names(name)`)で列挙できます。バンド／サブバンド型のプロセッサはインデックス付きの `band{i}.*` キーを列挙し、未知の名前には空配列を返します。一覧外のキーはプロセッサに無視され、そのキーを含むシーンを読み込むと [`Mixer.sceneWarnings()`](./mixing-scene-json.md) が報告します。上のソロプロセッサに加え、クリエイティブ FX 有効ビルドではリバーブやモジュレーションのインサート ID も使えます。
...
これらは [ミキシングシーン JSON](./mixing-scene-json.md) の `insert.processor` フィールドで使います。出荷される FX 有効の WASM ビルドでは、多くは一括マスタリングプロセッサでもあります。`effects.reverb.plate`、`effects.reverb.dattorro`、`effects.reverb.fdn`、`effects.reverb.velvet`、`effects.reverb.convolution` は `masteringProcessorNames()` から返り、一括適用パスで動作します。一方、幾何ベースやモジュレーション系のインサート — `effects.reverb.room`、`effects.acoustic.roomMorph`、`effects.modulation.ensemble` — はインサート専用で、`masteringProcessorNames()` には**現れません**。これらは `masteringInsertNames()` とシーンインサート経由で使ってください。
