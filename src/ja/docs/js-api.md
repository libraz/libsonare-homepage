...
| **編集 DSP** | `pitchCorrectToMidi`, `noteStretch`, `voiceChange`, `StreamingRetune` | ボーカル補正、ノート編集、ピッチ／フォルマント変更 |
...
| ボーカルやノートを編集したい | `pitchCorrectToMidi`, `noteStretch`, `voiceChange`, `StreamingRetune` | 解析ではなく音そのものを変える API です |
| 部屋の残響や明瞭度を測りたい | `analyzeImpulseResponse`, `detectAcoustic` | 楽曲ではなく録音空間を説明します |
...
```typescript
function version(): string  // 例: "1.2.1"
```
...
`analyzeImpulseResponse(...)` と `detectAcoustic(...)` は、曲そのものではなく録音空間を測る API です。きれいなインパルスレスポンスがある場合は前者、通常録音から大まかに推定したい場合は後者を使います。
...
RT60、EDT、C50、C80、D50、バンド別配列、信頼度の読み方は [ルーム音響解析](./acoustic-analysis.md) を参照してください。
...
| 配信向けラウドネス測定 | `lufs`, `momentaryLufs`, `shortTermLufs` | ITU-R BS.1770 / EBU R128 系のラウドネス値です。 |
...
`Audio` クラスは、よく使う単発ヘルパーをオブジェクト指向で呼ぶための wrapper です。サンプルとサンプルレートを内部に保持するので、毎回渡す必要がありません。`analyzeSections(...)`、`analyzeMelody(...)`、`analyzeDynamics(...)`、`analyzeTimbre(...)`、ルーム音響系の関数は、WASM wrapper ではスタンドアロン関数として呼び出します。
...
`StreamAnalyzer`、`Mixer`、`StreamingMasteringChain` などのクラスは、**embind**（Emscripten の C++↔JS ブリッジ）を通じて JavaScript に公開された C++ オブジェクトです。それぞれが WASM ヒープ上のメモリブロックを保持しますが、JavaScript のガベージコレクタはそれを認識・回収*できません*。そのため自分で解放する必要があります — `StreamAnalyzer` は `dispose()`、`Mixer` と `StreamingMasteringChain` は `delete()` を使います（一部の WASM クラスは `destroy()` も alias として公開します）。これを怠ると、長時間動くページで WASM メモリが少しずつリークします。`analyze()` のような通常の関数は普通の JS 値を返すので後始末は不要で、ハンドルを持つこれらのクラスだけが対象です。Node ネイティブの解放方法は異なるため、[ネイティブバインディング](./native-bindings.md) を参照してください。
:::
...
`StreamingEqualizer` は、ブロック単位で動かすリアルタイムセーフな EQ wrapper です。最大 24 バンド、zero-latency / natural / linear の phase mode、dynamic EQ、mid/side 処理、外部 sidechain、スペクトラム snapshot、オフラインの reference match を扱えます。WASM wrapper では先に `init()` を呼び、使い終わったら `delete()` で解放します。
...
`StreamingRetune` は libsonare 1.2.1 で追加された、ブロック単位のモノラル pitch retune wrapper です。grain と delay の状態を呼び出し間で保持するため、最初のブロック前に `prepare()`、使い終わったら `delete()` を呼びます。
