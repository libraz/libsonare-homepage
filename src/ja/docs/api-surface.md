...
| 構造とセグメンテーション | クロス類似度、recurrence／lag 行列、パス強調、サブセグメンテーション、凝集型クラスタリング、F0 トラックからのノートセグメンテーション | [JavaScript API](./js-api.md#librosa-互換ヘルパー), [Python API](./python-api.md#特徴抽出) |
| ビルドのイントロスペクション | 機械可読なケイパビリティカタログ（プロセッサ、パラメータの範囲と既定値、プリセット一覧）とビルド診断レポート | [JavaScript API](./js-api.md#capabilitycatalog), [Python API](./python-api.md#このビルドで何ができるか), [CLI](./cli.md#doctor) |
