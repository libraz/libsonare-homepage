...
WASM の多くの API はデコード済み PCM サンプルを受け取ります。エンコード済みバイト列を使う場合は、WAV/MP3 なら `Audio.fromMemory(...)`、ネイティブデコーダを試したあと AAC、OGG、FLAC などブラウザ対応形式を `AudioContext.decodeAudioData()` へフォールバックする場合は `Audio.fromMemoryWithBrowserFallback(...)` を使います。
...
上の例は独自の analyzer worklet を作るものです。トラックレーン、チャンネルストリップ、バス、MIDI クリップ、ライブ MIDI、楽器、キャプチャといったエンジン全体を worklet 内で動かしたい場合は、`@libraz/libsonare/worklet` の AudioWorklet bridge と `@libraz/libsonare/rt` の軽量 realtime module を使います。ブリッジの `SonareEngine` ファサードがそのエンジンを worklet へミラーします。詳しくは [リアルタイムとストリーミング](./realtime-streaming.md) を参照してください。
...
`StreamConfig.outputFormat` は、読み出す予定の転送形式を明示するために設定し、対応する読み出しメソッドを呼びます。

| `outputFormat` | 読み出しメソッド |
|----------------|----------------|
| `0` | `readFrames()` |
| `1` | `readFramesI16()` |
| `2` | `readFramesU8()` |

内部解析は float のままです。`readFramesI16()` / `readFramesU8()` は C++/WASM の読み出し経路で量子化（各 float をより小さい 16bit / 8bit 整数に詰める処理）を行うため、`postMessage` の前に JS 側で量子化する必要はありません。
...
WASM ラッパーが返すプレーンなリストやオブジェクトは、呼び出し元の JavaScript realm の `Array` / `Object` として再ルートされます。これにより、名前一覧ヘルパー（`*Names()`）、プリセット名ヘルパー、セクション結果、キー候補、`synthPresetPatch(...)` のオブジェクトは、手作業で作り直さなくても `structuredClone()` や `postMessage()` に渡せます。TypedArray のペイロードは、下の通常の transferable buffer ルールに従います。
