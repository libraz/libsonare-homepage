...
libsonare は <strong>音楽情報検索（MIR: Music Information Retrieval）</strong> とブラウザネイティブな音楽ツールのための、音声解析・マスタリング DSP ライブラリです。音声ファイルやストリームを入力すると、テンポ（BPM）、キー、コード進行、ビート位置、楽曲構造などの音楽的な情報を自動的に抽出できます。さらに、ラウドネス正規化、True Peak リミッター、EQ、ダイナミクス、ステレオ処理、リペア、リファレンスマッチング用のマスタリングプロセッサも含みます。

C++17 で書かれており高速に動作するほか、**WebAssembly** にコンパイルできるため、サーバーを介さずブラウザ上で直接同じ解析とマスタリングチェーンを実行できます。
...
- **音楽ツール制作者** — DAW プラグイン、コード検出器、自動採譜ツール、マスタリングワークフロー、音楽教育ソフトウェアを構築できます。libsonare が解析・マスタリングエンジンを提供するので、ユーザー体験の構築に集中できます。
...
::: tip ブラウザで試す
MIR 機能は [音声解析デモ](/ja/) で、アップロード不要の WAV マスタリングは [マスタリングデモ](/ja/mastering) で試せます。
:::
...
`StreamAnalyzer` は同じパイプラインを音声 <strong>チャンク単位</strong> で低レイテンシ実行し、リアルタイム可視化やライブ解析に適したフレーム単位の特徴量を出力します。Web Audio API の AudioWorklet と組み合わせて、ブラウザ内でリアルタイム処理できます。
...
現在のサイト同梱版では、WebAssembly / JavaScript アセットは圧縮前で合計約 508 KB です。内訳は [WebAssembly ガイド](/ja/docs/wasm#バンドルサイズ) を参照してください。
