...
このページは、各 DSP が内部で何をしているかを説明します。公開名の一覧である [マスタリングプロセッサ](./mastering-processors.md)、標準規格と互換性参照を分けてまとめた [アルゴリズム根拠](./algorithm-references.md)、テスト根拠をまとめた [実装検証](./implementation-validation.md) を補完するページです。UI にプロセッサを出す前、リアルタイム設定を選ぶ前、レンダリングレポートを説明するときの実装レベルの資料として使います。
...
| 設定フィールド | `TruePeakLimiterConfig` は `lookahead_ms`、`oversample_factor`、`apply_gain_at_input_rate` を持つ。`DenoiseClassicalConfig` は LogMMSE、MMSE-STSA、spectral subtraction、MCRA、IMCRA を持つ |
| 実行時契約 | 複数のプロセッサが、リアルタイムセーフなパラメータと、バッファ再確保や FIR カーネル再構築が必要な変更を明示している |
| 実装の include | リペア系では declick/declip が LPC ヘルパーを使う。denoise は `NoiseTracker` を使う。コンボリューションリバーブとリニアフェイズ EQ はパーティション畳み込みを使う |
...
| オーバーサンプリングとトゥルーピークフィルター | インターサンプルピーク推定とエイリアシング対策 | リミッター、クリッパー、サチュレーション、最終出力 |
...
MIR 側は、1 つの巨大なアナライザーではなく、再利用できる特徴量段の組み合わせで構成されています。STFT とフレーム処理が、メル／MFCC、クロマ、オンセット包絡、テンポグラム、ピッチ追跡、セクション特徴量の基盤になります。高レベル解析は、可能な範囲でこれらの表現を共有します。

初心者向けに言い換えると、解析はだいたい次の流れです。まず音声を短い窓に区切ります。次に各窓を周波数成分へ変換します。その結果から、リズムを見るならオンセット、ハーモニーを見るならクロマ、音色を見るならメルや MFCC、という別々の要約を作ります。BPM、キー、コードなどの高レベルな結果は、これらの要約から推定されます。
...
| 室内音響解析 | IR の energy decay curve 指標と、通常録音向けの自由減衰フィット | RT60、EDT、C50、C80、D50、バンド別減衰、信頼度 |

特徴量の逆変換とブラインド音響推定は有用ですが、どちらも推定です。逆変換ヘルパーは失われた位相やメル／MFCC の細部を復元できません。ブラインド音響モードは、通常録音に明確な自由減衰区間が含まれるとは限らないため、信頼度を返します。

## マスタリングチェーン

`masteringChain(...)` は、構造化された設定から決定的なプロセッサチェーンを組みます。

ストリーミング版も同じステージ順を使いますが、ブロック間でプロセッサ状態を保持します。
...
| `maximizer.truePeakLimiter` | オーバーサンプリングによるトゥルーピーク推定とシーリング制御 | インターサンプルピークを避ける | 計算コストと先読み系レイテンシがある |
...
| `repair.dereverbClassical` | `t60_sec`、`late_delay_ms`、over-subtraction、spectral floor、任意の WPE 設定を持つ STFT ドメインのデリバーブ設定 | 部屋鳴りやにじみの軽減 | ファイル単位 API として新しい `Audio` オブジェクトを返す |
...
| リバーブインサート | `SONARE_HAVE_FX` 有効時の `effects.reverb.plate` / `effects.reverb.dattorro`、`effects.reverb.fdn`、`effects.reverb.velvet`、`effects.reverb.convolution` | ミキサー／マスタリング insert graph 内の空間と残響 | アルゴリズムリバーブはストリーミング可能。コンボリューションはネイティブ insert 作成経路で IR を渡す必要があります |
...
ミキサーはルーティングであると同時に DSP です。チャンネルストリップは入力トリム、極性、アラインメントディレイ、パンロー、ステレオ幅、インサート処理、フェーダー、ミュート／ソロ、センド、メーターを扱います。バスはルーティングされた信号を加算し、VCA グループは制御ゲインを適用します。メーターはピーク、RMS、LUFS 系の値、トゥルーピーク、ゲインリダクション、相関値、ゴニオメーターバッファを計算します。
...
`RealtimeEngine` はトランスポート、テンポ同期、グラフ実行、クリップ再生、メトロノーム、キャプチャ、モニター実行、オフラインバウンス、テレメトリを担当します。これはマスタリングプロセッサレジストリの置き換えではなく、リアルタイムセーフな DSP ブロックを載せるスケジューラ兼グラフランタイムです。ファイル全体の解析、完全なリペア処理、ラウドネス最適化は、オフラインまたはレイテンシを許容する処理として扱います。

リアルタイム処理では「今この瞬間の音を、次の音が来る前に処理しきる」必要があります。そのため、ファイル全体を見てから判断する処理、巨大なバッファを作り直す処理、長い先読みを必要とする処理は向きません。逆に、EQ、簡単なゲイン、短いディレイ、状態を持ったコンプレッサーのような処理は、事前準備を済ませればリアルタイムに載せやすいです。
