...
| ルックアヘッドとスライディング最大値 | 先読みを使ったピーク制御 | ブリックウォールリミッター、True Peak リミッター、マキシマイザー |
| オーバーサンプリングと True Peak フィルター | サンプル間ピーク（ISP）推定とエイリアシング対策 | リミッター、クリッパー、サチュレーション、最終出力 |
...
| パーティション畳み込み | 長いカーネルやインパルス応答（IR）をブロック単位で効率よく処理 | コンボリューションリバーブ、リニアフェイズ EQ、マッチ EQ |
...
- **オーバーサンプリング＆True Peak（トゥルーピーク）フィルター** — 一時的に高いサンプルレートで処理し、*サンプル間ピーク*（サンプルの間に現れ、DAC が実際に再構成するピーク）を捉え、非線形処理で生じる歪みが可聴帯域に折り返す（エイリアシング）のを防ぎます。
- **ミッド/サイド**（M/S） — ステレオ信号を*ミッド*（モノラルの中央）と*サイド*（左−右の差分）に変換し、中央と広がりを別々に処理してから戻します。
- **パーティション畳み込み** — 畳み込みはある信号の「指紋」を別の信号に適用する処理です（部屋のインパルス応答やリニアフェイズ EQ など）。長いフィルターをブロックに分割し、リアルタイムでも効率よく動かします。
...
解析側、すなわち MIR（music information retrieval。テンポ、キー、コードといった音楽的な情報を音声から取り出すこと）は、1 つの巨大なアナライザーではなく、再利用できる特徴量段の組み合わせで構成されています。STFT（短時間フーリエ変換）とフレーム処理が、メル／MFCC、クロマ、オンセット包絡線、テンポグラム、ピッチ追跡、セクション特徴量の基盤になります。高レベル解析は、可能な範囲でこれらの表現を共有します。
...
| メル / MFCC | メルフィルタバンクと DCT 系のケプストラム圧縮。任意でケプストラルリフタリング（`lifter`。既定は 0 で無効） | ML 特徴量、音色サマリー、librosa 互換ワークフロー |
| クロマ / CQT / VQT / NNLS クロマ | ピッチクラス表現と対数周波数表現。定Qクロマグラム（`chromaCqt`）を含む | キー、コード、ハーモニー類似度、ピッチ意識の特徴量 |
| オンセット / テンポグラム / PLP | オンセット包絡線と局所周期性の表現 | BPM、ビート、リズム、パルス解析 |
| ピッチ追跡 | YIN / pYIN 系の F0 推定 | メロディ抽出と単音ピッチ解析 |
| 逆変換特徴量 | 非負最小二乗（NNLS）によるメルフィルタバンク逆変換、逆 DCT による MFCC の復元、Griffin-Lim 音声合成 | デバッグ用プレビューと特徴量の往復確認 |
...
プリセットは別の DSP アルゴリズムではありません。リペア、EQ、ダイナミクス、ステレオ処理、True Peak リミッター、ラウドネス最適化を、ジャンルや配信先の既定値で組み合わせた名前付き設定です。
...
<FlowDiagram
  title="マスタリングチェーンのステージ順"
  :nodes="[
    { id: 'repair', label: 'リペア', col: 0, row: 0, group: 'clean' },
    { id: 'eq', label: 'EQ', col: 1, row: 0, group: 'tone' },
    { id: 'dyn', label: 'ダイナミクス', col: 2, row: 0, group: 'tone' },
    { id: 'stereo', label: 'ステレオ処理', col: 3, row: 0, group: 'image' },
    { id: 'tp', label: 'True Peak リミッティング', col: 4, row: 0, group: 'output' },
    { id: 'loud', label: 'ラウドネス最適化', col: 5, row: 0, group: 'output', variant: 'success' }
  ]"
  :edges="[
    { from: 'repair', to: 'eq' },
    { from: 'eq', to: 'dyn' },
  ...
  :groups="[
    { id: 'clean', label: '整音' },
    { id: 'tone', label: '音色とレベル' },
    { id: 'image', label: '定位' },
    { id: 'output', label: '出力' }
  ]"
  caption="同じ設定なら常にこのステージ順になります。プリセットが変えるのは各ステージの設定値であって、順番ではありません。"
/>
...
| `dynamics.compressor` | フィードフォワード検出、アタック／リリースエンベロープ、レシオ／ニーのゲイン計算、メイクアップゲイン | マスター全体のダイナミクスを整え、一体感を出す | パラメータ変化は平滑化してジッパーノイズを避ける |
...
| プロセッサ | 実装 | 主な用途 | リアルタイム上の注意 |
|------------|------|----------|----------------------|
| `eq.apiStyle` | 一般的なバンド定義を受ける API 向け EQ アダプター | UI から扱いやすい安定した EQ 入口 | 実際の EQ 設計へ委譲する |
...
| `maximizer.loudnessOptimize` | ラウドネスを測定し、目標に向けたゲイン戦略を適用 | 配信／放送向けの LUFS（フルスケール基準のラウドネス単位）目標へ合わせる | ファイル全体の最適化はオフライン処理 |
| `maximizer.maximizer` | シーリング管理付きの音圧向けリミッティング | 知覚上の音量を上げる | 強い設定では後段の True Peak 保護が重要 |
| `maximizer.softKneeMax` | ハードな上限前にソフトニーのリミッティング曲線を使う | 急なリミッティング感を抑える | 最終 True Peak 保護の前段に向く |
| `maximizer.truePeakLimiter` | オーバーサンプリングによる True Peak 推定とシーリング制御 | サンプル間ピークを避ける | 計算コストと先読み系レイテンシがある |
...
| プロセッサ | 実装 | 主な用途 | リアルタイム上の注意 |
|------------|------|----------|----------------------|
| `saturation.ampSim` | プリアンプドライブ → トーンスタック → 任意のパワーアンプ（sag／トランス／NFB） → キャビネット特性 | ギター／ベースアンプ系の色付けインサート | `cab`／`cabModel`／`ampModel` の離散トポロジー選択はオートメーションではなく構築時に指定する |
...
|-----|------|----------|----------------------|
| HPSS / harmonic / percussive | スペクトル表現上のメディアンフィルターで、水平成分の倍音エネルギーと垂直成分の打撃エネルギーを分離 | リミックス、前処理、解析の整理 | ファイル全体のオフライン処理向き |
...
| モジュレーション／ディレイインサート | コーラス、アンサンブル、フランジャー、フェイザー、ワウ、オートワウ、ロータリー、リングモジュレーター、ピッチシフター、ステレオディレイの DSP はソースモジュールとして存在 | クリエイティブ FX の構成要素 | インサートファクトリとシーンインサート経由で公開。トップレベルの単独 JS/Python ヘルパー関数ではない |

::: info リバーブインサートの提供条件
`effects.reverb.plate` / `effects.reverb.dattorro`、`effects.reverb.fdn`、`effects.reverb.velvet`、`effects.reverb.convolution`、モジュレーション／ディレイインサートには `SONARE_HAVE_FX` が必要です。`effects.reverb.room` と `effects.acoustic.roomMorph` には `BUILD_ACOUSTIC_SIM` も必要です。
...
バスはルーティングされた信号を加算し、VCA グループは制御ゲインを適用します。メーターはピーク、RMS、LUFS 系の値、True Peak、ゲインリダクション、相関値、ゴニオメーターバッファを計算します。
...
ファイル全体の解析、重いリペア処理、ラウドネス最適化は、オフラインまたはレイテンシを許容する処理として扱います。
