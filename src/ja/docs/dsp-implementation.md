...
| 設定フィールド | `TruePeakLimiterConfig` は `lookahead_ms`、`oversample_factor`、`apply_gain_at_input_rate` を持つ。`DenoiseClassicalConfig` は 3つのゲイン関数（LogMMSE、MMSE-STSA、spectral subtraction）と 3つのノイズ推定器（Quantile、MCRA、IMCRA）を持つ |
