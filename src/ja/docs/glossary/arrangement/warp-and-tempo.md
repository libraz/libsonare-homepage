...
テンポマップは **テンポセグメント** のリストです。各セグメントは「タイムラインのこの地点以降、テンポは何 BPM」を表します。曲は単一の一定テンポでも、複数のセグメント — 遅いイントロ、速い A メロ、最後のリタルダンド — でもかまいません。セグメントは **ランプ** することもでき、BPM を急に切り替えるのではなく滑らかに変化させられます。
...
すべてのオーディオクリップは **ワープモード** を持ちます。ワープモードは、そのクリップがグリッドに *どのように* 追従するか、そもそも追従するかどうかを決めます。
...
- **tempo-sync** — **元の音程を保ったまま** テンポに追従。タイミングがグリッドに合うようクリップを時間伸縮しつつ、フェーズボコーダ（周波数領域で音声を伸縮するアルゴリズム）が音程を一定に保ちます。ボーカルやメロディックなループなど、ノートが音程を保たねばならないものに使います。
:::
...
<WarpMapFigure
  title="アンカーが留め、その間が伸縮する"
  :labels="{
    sourceLane: '録音されたままのソース音声',
    timelineLane: 'プロジェクトのタイムライン',
    marker: 'ワープアンカー',
    stretched: '引き伸ばされる',
    compressed: '縮められる',
    grid: '拍グリッド',
  }"
  caption="アンカーはソース上の 1 点をタイムライン上の 1 点に固定します。アンカーとアンカーのあいだにある音声は、グリッドが与える長さに合わせて詰め込まれます。走ったフレーズは引き伸ばされ、もたついたフレーズは縮められる。しかもそれが 1 つのクリップの中で、ソースファイルには一切手を加えずに起きます。"
/>

::: tip アンカーを持つのはアプリ側
エンジンはアンカーを読むだけで、自分で作り出すことはありません。各アンカーはサンプル位置どうしの絶対的な対応なので、クリップは渡されたマップが示すぶんだけグリッドに寄り、後からテンポマップを編集してもマップが書き換わることはありません。ユーザーがテンポを変えられるホストアプリは、アンカーを計算し直して新しいワープマップを渡す必要があります。アンカーが 2 個未満のマップは伸縮を表せないため、`repitch` クリップはネイティブ速度で再生され、`tempo-sync` クリップはコンパイルに失敗します。
:::
