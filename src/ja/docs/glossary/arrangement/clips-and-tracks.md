...
<FlowDiagram
  title="プロジェクト → トラック → クリップ → 楽器"
  direction="TB"
  :nodes="[
    { id: 'project', label: 'プロジェクト', col: 1, row: 0, variant: 'accent' },
    { id: 'vocals', label: 'オーディオトラック: Vocals', col: 0, row: 1 },
    { id: 'drums', label: 'オーディオトラック: Drums', col: 2, row: 1 },
    { id: 'bass', label: 'MIDI トラック: Bass', col: 3, row: 1 },
    { id: 'clipVerse', label: 'クリップ: Aメロのボーカル', col: 0, row: 2 },
    { id: 'clipChorus', label: 'クリップ: サビのボーカル', col: 1, row: 2 },
    { id: 'clipDrum', label: 'クリップ: ドラムループ', col: 2, row: 2 },
    { id: 'clipBass', label: 'クリップ: ベースライン', col: 3, row: 2 },
    { id: 'destination', label: 'MIDI デスティネーション', col: 3, row: 3, variant: 'accent' },
    { id: 'instrument', label: '楽器: NativeSynth / SoundFont', col: 3, row: 4, variant: 'success' }
  ]"
  :edges="[
    ...
    { from: 'bass', to: 'destination', label: 'ルーティング' },
    { from: 'destination', to: 'instrument', label: 'レンダリング' }
  ]"
  caption="オーディオトラックはすでに音を保持しているため、そのまま再生されます。一方、MIDI トラックのクリップが持つのはノートだけなので、トラックは別途、楽器にバインドされたデスティネーションへルーティングし、バウンス時にそこで音がレンダリングされます。"
/>
...
その何かに届けるのが、トラックの **MIDI 送出先（デスティネーション）** です。デスティネーションは番号の付いたルーティング枠です。**内蔵シンセサイザー（NativeSynth）** や **SoundFont** プレイヤーといった楽器をデスティネーションにバインドし、トラックをそこへルーティングすると、トラックのノートがその楽器へ送られ、実際の音声が合成されます。

| 手順 | やること | 結果 |
|------|---------|------|
| 1 | MIDI トラックとクリップを追加 | ノートはあるが無音のレーン |
| 2 | クリップにノートイベントを入れる | 演奏内容が書き込まれる |
| 3 | トラックをデスティネーションへルーティング | ノートに行き先ができる |
| 4 | そのデスティネーションに楽器をバインド | レンダリング時にノートが音になる |

この分離は強力です。同じ MIDI クリップを、デスティネーションの楽器を入れ替えるだけで、今日はシンセ、明日はサンプル楽器で鳴らせます。
...
**トラック操作** — レーンそのものの管理: トラックの追加・削除・改名、ルーティング（ミキサーストリップや出力へのバインド、MIDI デスティネーションの設定）、そしてオーディオと MIDI の種別の変更です。
