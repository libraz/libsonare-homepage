...
`BoundaryDetector` は MFCC とクロマ特徴量を組み合わせ、L2 正規化し、cosine 自己類似度行列を作り、checkerboard novelty curve から境界ピークを選びます。`SectionAnalyzer` は境界を区間へ変換し、RMS エネルギー、クロマ記述子、スペクトル平坦度、ボーカル帯域エネルギーを計算したうえで、`Intro`、`Verse`、`Pre-Chorus`、`Chorus`、`Bridge`、`Instrumental`、`Outro`、`Unknown` のラベルと信頼度を割り当てます。
:::
