...
**ピッチ補正**は、音を現在のピッチから目標へ寄せます。**ノート編集**は、選んだ区間（伸ばした 1 音、タイミングの調整）を、他に触れずに整形します。どちらもクリップ全体の変換ではなく、区間を意識した外科的な編集です。本ページは libsonare の `PitchCorrector` と `NoteEditor` に根拠づけます。用語は [編集の基礎](../concepts/editing-basics.md) を読んでください。
