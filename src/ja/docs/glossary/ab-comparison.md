...
A/B では「大きくなったか」ではなく、問題が減ったかを確認します。低域が安定したか、ボーカルや主旋律が埋もれにくくなったか、シンバルや歯擦音（サ行の耳につく成分）が痛くないか、サビでリミッターが潰していないかを順番に見ます。
...
別環境での確認も同じ考え方で進めます。スマートフォン・安いイヤホン・車内スピーカー・小音量再生では、ラウドネスマッチングを外して実音量で聴いた方が、再生環境ごとの聴こえ方の差を把握しやすくなります。スタジオ環境では成立していたバランスが他環境で崩れる典型例は、低域の量と歯擦音まわりです。

:::: details 実装メモ

デモではレンダリング後の入力 LUFS（Loudness Units relative to Full Scale）と出力 LUFS の差を使い、再生時のゲインで処理前 / 処理後をそろえます。音声ファイル自体を書き換えるのではなく、比較時の再生音量を調整する方式です。
