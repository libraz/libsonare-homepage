...
| ステレオの広がりが有効で、モノラル再生でも崩れないか | Stereo width、Mid/Side、相関の確認 |
| 変換や再生時にクリップしないか | True Peak リミッターと ceiling 管理 |
...
Quick Master は、実用的な初回処理のための流れです。

1. 元音源を読み込む。
2. 音源の種類を選ぶ。
3. 配信先のラウドネスターゲットを選ぶ。
4. レンダーし、ラウドネスマッチ付きで Before / After を比較する。
...
Quick Master はこのチェーンに実用的な初期値を入れます。Studio は同じチェーンをモジュールとして直接編集できるようにします。どちらも最後は同じレンダリングフローに入るため、違いは処理エンジンではなく、触れる深さです。
