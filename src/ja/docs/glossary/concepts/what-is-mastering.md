...
マスタリングは、ミックスが終わった後に行う最終的な品質確認と配信用の仕上げです。完成したミックスを、配信サービス、スマートフォン、ヘッドホン、スピーカー、車内再生、コーデック変換をまたいでも意図どおりに聴こえる状態、つまり再生互換性のある状態に整えます。
...
| 問い | マスタリングでの処理 |
|------|----------------------|
| 公開先に対して音が小さすぎる、または大きすぎないか | [ラウドネス](../lufs.md)測定と正規化 |
| 全体の音色が暗い、刺さる、薄い、膨らみすぎていないか | 広い EQ、Tilt EQ、トーン補正 |
| 動きが大きすぎる、または平坦すぎないか | 穏やかなダイナミクス制御とリミッター |
| ステレオの広がりが有効で、[モノラル再生でも崩れないか](./mono-compatibility.md) | ステレオ幅、Mid/Side（中央成分とサイド成分）、相関の確認 |
| 変換や再生時にクリップしないか | [True Peak](../true-peak.md) リミッターとシーリング管理 |
...
Studio モードでは、同じ考え方を「リペア・EQ・ダイナミクス・[Air バンド](../air-band.md)・ステレオイメージ・リミッター・ラウドネス目標・出力」というモジュールとして確認できます。

## マスターの判断方法

処理が良くなったか判断するときは、[ラウドネスマッチング](./loudness-matching.md)を効かせた A/B 比較を使います。音が大きい方は、数秒だけ聴くと良く聞こえがちだからです。

<SonareDemo id="loudness-meter" />

そのうえで、次の 3 つを確認します。

- **再生互換性:** 小さなスピーカーやヘッドホンでも成立しているか。
