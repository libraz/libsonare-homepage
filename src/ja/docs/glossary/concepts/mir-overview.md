...
MIR は **Music Information Retrieval**（音楽情報検索）の略で、音を*音楽的な*答えに変える音声解析の領域です — テンポ、ビート位置、キー、コード、ピッチ、音色、構造。このページは地図です。ドキュメント全体で出会う用語をまとめ、それらがどう積み上がっているかを示すので、どの機能をどこで呼べばよいかが分かります。
...
| 録音空間はどんな響きか? | 室内音響解析 | インパルスレスポンス減衰、またはブラインド自由減衰推定 |
...
どこでも繰り返し出てくるパラメータが 2 つあります — `nFft`（ウィンドウサイズ。大きいほど周波数分解能は細かいが時間はぼやける）と `hopLength`（ウィンドウ間のステップ。小さいほどフレームが増え動きが滑らかになる）。周波数分解能と時間分解能のトレードオフは libsonare 固有の癖ではなく、本質的なものです。

## 知覚特徴量: メル、MFCC、CQT、VQT

**メルスペクトログラム**は周波数分解能を人間の聴覚に合わせてひずませます — 低域は細かく、高域は粗く。**MFCC** は log-mel パワースペクトルの DCT で、スペクトル包絡を近似する無相関な係数（コンパクトな「音色の指紋」）を生みます。**CQT** と **VQT** は*音楽的に*等間隔なビン（半音ごと）を使うため、Hz 等間隔よりピッチ関係そのものが重要な場面で活きます。
...
室内音響解析は MIR に隣接する領域です。音符、リズム、楽曲構成ではなく、録音に含まれる空間の性質を記述します。クリーンなインパルスレスポンスがある場合は、RT60、EDT、C50、C80、D50、バンド別減衰を IR 解析で測定します。通常録音しかない場合は、ブラインド音響推定で残響時間の手がかりと信頼度を得られます。詳しくは [室内音響解析](../../acoustic-analysis.md) を参照してください。
...
関連: [イントロダクション](../../introduction.md), [オーディオ基礎](./audio-basics.md), [JavaScript API](../../js-api.md#特徴抽出), [室内音響解析](../../acoustic-analysis.md), [DSP 実装解説](../../dsp-implementation.md), [librosa 互換性](../../librosa-compatibility.md)
