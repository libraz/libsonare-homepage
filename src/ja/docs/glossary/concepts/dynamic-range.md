...
下のデモに出る LRA（ラウドネスレンジ）は、これを数値にした読み値です。クリップ全体でラウドネスがどれだけ動くかを測るので、平坦にしたマスターほど小さくなります。メーターの横の曲線はその動きを時間軸で見たものです。クレストファクターと相関値は別の読み値で、ここには表示されません。
...
処理後にサビの持ち上がりが小さくなった、キックが丸くなった、ボーカルの子音だけが前に飛び出した、静かな箇所の空気感が消えた──こうしたサインが出たら、ダイナミックレンジを削りすぎている可能性が高いです。[LUFS](../lufs.md)（フルスケール基準のラウドネス単位）が上がっただけで改善と判断せず、ラウドネスマッチした A/B で楽曲のうねりが残っているかを確認します。
