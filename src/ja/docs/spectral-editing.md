...
`spectralEdit(...)` は、libsonare の領域指定 STFT エディタです。STFT（短時間フーリエ変換）は、周波数ごとの編集を可能にするために信号を切り分ける「時間 x 周波数」のグリッドで、詳細は[用語集の解説](./glossary/analysis/spectrogram-stft.md)を参照してください。モノラルクリップ内の「時間 x 周波数」の矩形を変えたい場合に使います。笛鳴りを弱める、短いハムを消す、狭い帯域を持ち上げる、小さな欠落を近傍フレームから補修する、といった用途向けです。
...
領域はサンプル位置と Hz で指定しますが、実際の編集は次の 2 つのオプションが作る STFT のグリッド上で行われます。

<StftFramingFigure
  title="nFft と hopLength が作るもの"
  :labels="{
    signal: '入力サンプル',
    windows: '窓掛けされたフレーム',
    frames: 'スペクトログラムの列',
  ...
  caption="nFft は 1 つの解析窓の長さ、hopLength は次の窓を何サンプル先から始めるかで、既定値では窓が 4 分の 3 ずつ重なります。1 つの窓がスペクトログラムの 1 列になり、この縦の列に時間範囲が、その列を分割した周波数ビンに lowHz/highHz の範囲が対応します。"
/>
