...
| 内蔵シンセサイザー（NativeSynth）のボイス調整 | `tools/voicematch/` が仮実装の GM フォールバックボイスと、ドライな FluidSynth/SoundFont 側の参照音をレンダーして音色指標を比較。`autofit.py` は隔離されたビルドディレクトリで、指定した数値定数を自動調整できる。これは調整用ハーネスであり、物理モデルが完成済みであることの証明ではない |
...
| パフォーマンス | `benchmarks/*.cpp`、`benchmarks/results.json`、`benchmarks/results_cpp.json` でスペクトル処理、ストリーミングメル／クロマ、マスタリング補助処理、サンプル間ピーク（ISP）検出、ステレオ処理、ミキシング、EQ、リサンプリング周辺のホットパスを確認 |
...
一方で libsonare では、リアルタイムセーフなブロック処理とオフライン用ヘルパーを区別しています。重いリペア処理やファイル全体のラウドネス最適化は、意図的にオフライン処理として設計されています。

## ボイス調整ハーネス

内蔵シンセサイザーの物理モデル系ボイスは、音色調整中の**仮実装**として文書化しています。その作業のために、リポジトリには `tools/voicematch/` があります。

- `voicematch.py` は libsonare の GM フォールバック側と参照側をレンダーし、倍音バランス、ピッチ誤差、トーナル成分とノイズ成分の比、エンベロープ関連の測定値を比較します。
- `autofit.py` は、試す数値定数を JSON spec から読み、隔離された共有ライブラリをビルドして比較を再レンダーし、ミスマッチが小さくなる値を探索します。中断時はソーステキストを戻し、`--dry-run` でも実行できます。

これらはキャリブレーション変更の判断材料です。存在するからといって、ヴァイオリン、金管、リード、フルート、ピアノ、ギター、ベースのモデルが本番品質の楽器シミュレーションとして完成している、という意味ではありません。

## 精度の境界

librosa 互換性は、対象ヘルパーが生成済み参照値と比較されていることを意味します。高レベルの音楽解析結果がすべて librosa と同一という意味ではありません。

マスタリング検証では、DSP の不変条件、ラウドネス／True Peak 挙動、ゴールデンハッシュ、必要に応じた公開アルゴリズム上の前提を確認します。
