...
| `analysis/acoustic_analyzer.*` | [ルーム音響解析](./acoustic-analysis.md) |
...
│   ├── api/            # チェーン・レジストリ・プリセット（25 種）
│   ├── eq/ dynamics/ spectral/ stereo/   # プロセッサーファミリー
...
::: details フェーズボコーダーとは？
フェーズボコーダーは、目立ったアーティファクトなしに音声をタイムストレッチ（リサンプリングと組み合わせればピッチシフト）する標準的な手法です。STFT を取り、再構成の前に各周波数ビンの*位相を新しい時間軸に合わせて進める*ことで、ピッチやスペクトルの質感を保ったまま音を長く・短くできます。libsonare は `timeStretch` / `pitchShift` や編集 DSP のボイス系ツールで使っています。
:::
...
```
出力: ~1,607 KB WASM（gzip ~573 KB）＋ JS グルー。
      合計 ~1,721 KB（gzip ~598 KB）— src/wasm/meta.json を参照
