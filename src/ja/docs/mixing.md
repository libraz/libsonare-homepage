...
ストリップのグループとグループゲインは `addVcaGroup(id, gainDb, members)` で追加し、`setVcaGroupGainDb(...)` で調整します。このグループ定義（ゲインとメンバー構成）はシーン JSON を往復します。`setVcaOffsetDb(...)` による調整は、現在のセッション限りの**ライブ**なストリップごとのオフセットで、シーンには*保存されません*。
