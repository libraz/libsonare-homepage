...
| 音声を編集・変換する | `hpss`, `timeStretch` / `time_stretch`, `pitchShift` / `pitch_shift`, `trimSilence` / `trim_silence` | フェーズボコーダーやリサンプリングの細部は librosa と異なります |
...
libsonare リポジトリには、STFT、Mel/MFCC、chroma、CQT、pitch、onset/beat/tempo、tempogram/PLP、PCEN、dB 変換、framing/sequence helper、silence trim/split、HPSS、harmonic/decompose/remix、tonnetz などについて librosa 参照比較テストがあります。
...
| `librosa.pyin()` | `pitchPyin()` / `pitch_pyin()` | 確率的 YIN |
...
::: info ストリーミング
さらに、`StreamAnalyzer` によるリアルタイムのプログレッシブ BPM／キー／コード推定も libsonare 固有の機能です（librosa にはストリーミング API はありません）。
:::
...
| `fmax` | sr/2 | sr/2 |
| `n_mfcc` | 20 | WASM/Node JS ヘルパーは 13。Python トップレベルの `mfcc()` は 20 |
| `n_chroma` | 12 | 12 |
...
*Apple M5 Max（16 コア、128 GB）、73 秒 WAV（44100 Hz ステレオ）で計測。すべての機能別呼び出しは生音声からのスタンドアロン計測です。完全な計測手法と再現手順は [ベンチマーク](/ja/docs/benchmarks) を参照。*
