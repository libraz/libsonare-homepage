...
| `librosa.effects.trim()` | `trimSilence()` / `trim_silence()` | `(audio, startSample, endSample)` を返す |
| `librosa.effects.split()` | `splitSilence()` / `split_silence()` | 非無音区間 |
...
| `librosa.feature.tempogram()` | `tempogram()` | 自己相関ベースのテンポグラム |
...
*Apple M5 Max（16コア、128GB）、73秒 WAV（44100 Hz ステレオ）で計測。すべての機能別呼び出しは生音声からのスタンドアロン計測です。完全な計測手法と再現手順は[ベンチマーク](/ja/docs/benchmarks)を参照。*
