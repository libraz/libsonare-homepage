...
要点だけ言うと、コアのスペクトル変換（STFT・メル・MFCC）は librosa とほぼ同等で、反復的なアルゴリズム（HPSS・pYIN）と完全解析パイプラインは大幅に高速です。機能別の数値、ハードウェア構成、計測手法、再現手順は 1 か所にまとめてあります（[ベンチマーク](/ja/docs/benchmarks) を参照）。このページに数値の複製を置かないのは、更新時のズレを防ぐためです。
