...
**Q因子が周波数ビンごとに変化する**CQTの変種（gammaパラメータで制御）。標準CQTよりも柔軟に時間分解能と周波数分解能のトレードオフを調整できます。

::: details CQT vs VQT
| 特徴 | CQT | VQT |
|---------|-----|-----|
| Q因子 | 一定 | 可変（gammaパラメータ） |
| 低周波数の分解能 | 高い周波数分解能、低い時間分解能 | 調整可能 |
| 用途 | 音楽採譜 | 適応的解析 |
:::
...
足でタップするリズミカルなパルス。ビート検出は各ビートの**推定タイムスタンプ**を見つけます。
...
音の「<strong>色</strong>」 - 同じ音の高さと大きさでも、ピアノとギターの音を区別するもの。
