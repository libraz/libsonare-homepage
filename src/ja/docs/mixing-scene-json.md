...
インサートの `slot` とセンドの `timing` は**必ず文字列で指定してください**。パーサは値が JSON 文字列のときだけ読み取るため、`"timing": 1` のような数値は黙って破棄され、フィールドは既定値に戻ります。常に `"pre"` か `"post"` と書いてください。
:::
...
| `drumBusSubgroup` | キック／スネア／オーバーヘッドをグループバス（ロール `subgroup`）へ送り、パラレルコンプとテープでまとめ、「drums」VCA で調整 |
...
::: warning `eq.parametric` インサートは `highPassHz` / `presenceDb` を無視する
このプリセットはそのまま掲載していますが、`eq.parametric` の 2 つのパラメータは実際には**何もしません**。同梱の `eq.parametric` インサートが読むのは**バンドインデックス指定の数値キーだけ**です（バンドごとの周波数・ゲイン・Q をインデックスで指定）。そのため `highPassHz` や `presenceDb` のような名前付きキーは黙って無視され、ここでは効果を持ちません。

シーンのインサートを通じて実際に効くトーン調整には、次を使ってください。

- `eq.tilt` の `tiltDb` — 明暗を広く傾けるティルト。
- `spectral.airBand` の `airDb` — 高域シェルフによる「エア」の持ち上げ。

つまり、このプリセットの EQ パラメータはシーンの*形*を示す例示であって、機能するハイパス＋プレゼンスブーストではないと捉えてください。
:::
