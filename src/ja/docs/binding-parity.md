...
| `analyze(...)` の戻り値 | すべてのバインディング（C ABI・Python・Node ネイティブ・WASM）が完全な `analyze` 結果を返す。コード、セクション、音色、ダイナミクス、リズム、メロディー、フォーム、ビートごとの強さが含まれ、ランタイム間でフィールド集合が揃う |
...
::: info 詳細な解析フィールド
`analyze(...)` の結果には、すべてのバインディングでコード、セクション、音色、ダイナミクス、リズム、メロディー、フォーム、ビートごとの強さが含まれます。

1 つのファミリーだけが必要なときは、ランタイム共通で focused helper も使えます。
