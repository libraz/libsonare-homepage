...
| `analyze(...)` の戻り値 | WASM は `quick::analyze` 由来の完全な形を返す。Python・Node ネイティブ・C ABI は C API のコンパクトな結果を返す |
...
::: info 詳細な解析フィールド
WASM の完全な `analyze(...)` には、コード、セクション、音色、ダイナミクス、リズム、フォームが含まれます。

Python、Node ネイティブ、C ABI でそれらを得る場合は、focused helper を呼びます。
