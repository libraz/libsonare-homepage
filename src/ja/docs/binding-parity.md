...
| 組み込み楽器（NativeSynth の preset/patch） | 対応 — [組み込み楽器](./native-synth.md)を参照 | 対応 |
...
| `analyze(...)` の戻り値 | C ABI・Node ネイティブ・WASM は完全な `analyze` 結果を返す。コード、セクション、音色、ダイナミクス、リズム、メロディー、フォーム、ビートごとの強さが含まれる。Python だけは例外で、その `AnalysisResult` は BPM、BPM の確信度、キー、拍子、ビート時刻のみを持つ。ほかのファミリーは専用関数（`detect_chords`、`analyze_sections` …）を使う |
...
::: info 詳細な解析フィールド
C ABI・Node ネイティブ・WASM では、`analyze(...)` の結果にコード、セクション、音色、ダイナミクス、リズム、メロディー、フォーム、ビートごとの強さが含まれます。Python だけは例外で、その `analyze(...)` は BPM、BPM の確信度、キー、拍子、ビート時刻のみを返します。

1 つのファミリーだけが必要なとき、または Python でコア要約以外が必要なときは、ランタイム共通で focused helper も使えます。
