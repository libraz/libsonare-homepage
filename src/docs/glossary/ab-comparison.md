...
Do not ask only whether the master is louder. Ask whether the problems are smaller. Check whether the low end is steadier, the vocal or lead part is easier to follow, cymbals and sibilants (the sharp "s" sounds of a vocal) remain comfortable, and the loudest chorus is not being pinned by the limiter.
...
Cross-environment checks follow the same logic. Phone speakers, cheap earbuds, car stereos, and low-volume playback are easier to judge with matching off, because translation issues — whether the master still holds up on another playback system — are about absolute loudness, low-end balance, and sibilance — not the comparison fairness that matching exists to enforce.

:::: details Implementation notes

The demo compares input and output LUFS (Loudness Units relative to Full Scale) after rendering, then applies playback gain to the louder side during A/B playback. It does not rewrite either audio file for comparison.
