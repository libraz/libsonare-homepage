...
libsonare builds an energy decay curve from the input and fits room parameters (RT60, clarity, per-band absorption, volume, DRR) to it. In impulse-response mode the decay curve comes straight from the supplied IR; in blind mode it is recovered by locating segments where the reverberant tail is briefly audible — note releases, transient gaps, silences — and stitching an estimate of the decay from them. The confidence score weights two things drawn from the fit itself: how closely RT60 and EDT agree — a decay that reads the same length from its full 60 dB extrapolation as from its first-10 dB slope is a decay the fit trusts — and how much clean decay range was actually captured before the fit had to stop. RT60/EDT agreement is the dominant signal, so confidence scales naturally from high on a studio-measured sweep, where the two measures line up and the tail runs clean and long, to low on a loud, dense, heavily compressed master, where RT60 and EDT diverge and only a shallow window of decay was ever visible.
:::
