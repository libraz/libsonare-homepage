...
Only the built-in waveform synth (`setBuiltinInstrument`) offers true MPE-style (MIDI Polyphonic Expression) per-note expression — per-note pitch bend and channel/polyphonic pressure, plus full 16-bit MIDI 2.0 velocity — and its MPE bend range is fixed at ±2 semitones. NativeSynth (`setSynthInstrument`) works per channel instead: RPN 0 configures its bend range, but it acts on no other RPN, consumes no NRPN value, and has no generic 14-bit CC pairing — velocity and MIDI 2.0 CC values are both quantized down to 7 bits, and per-note pressure is not tracked. High-resolution RPN/NRPN and 14-bit CC handling lives one level up, in the engine's `bindMidiCcBinding(...)` parameter layer described below, not in the instrument. See [Native Synth](./native-synth.md) for NativeSynth's per-channel bend and CC details.
:::
...
- **Live SysEx** — `pushMidiSysex(destinationId, data, renderFrame = -1)` queues a full SysEx (System Exclusive, a free-form manufacturer-specific message) frame to a destination; `data` is the complete message including the leading `0xF0` and trailing `0xF7` (1..512 bytes), and `renderFrame` follows the same immediate/schedule convention as the other `pushMidi*` calls. Its primary use is delivering a GS/GM reset or a GS insertion-effect (EFX) selection to a live SF2-bound destination without stopping playback — see [SoundFont Player](./soundfont-player.md) for what that SysEx selects.
- **A single UMP word** — `pushMidiUmp(destinationId, word0, renderFrame = -1)` queues one MIDI 1.0 channel-voice message packed as a 32-bit UMP (Universal MIDI Packet, the MIDI 2.0 message format) word, dispatched immediately. This is the compact way to restore program, pitch-bend, and pressure state on a transport seek: pack each message into a word and push them, rather than fanning out to a different `pushMidi*` call per message type.
