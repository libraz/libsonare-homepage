...
- **A single UMP word** — `pushMidiUmp(destinationId, word0, renderFrame = -1)` queues one MIDI 1.0 channel-voice message packed as a 32-bit UMP word, dispatched immediately. This is the compact way to restore program, pitch-bend, and pressure state on a transport seek: pack each message into a word and push them, rather than fanning out to a different `pushMidi*` call per message type.
