...
| Mastering limiter options | `releaseMs` / `release_ms` and `applyGainAtInputRate` / `apply_gain_at_input_rate` are available on the mastering helper surfaces. A zero release keeps the 50 ms library default on the simple one-shot helper; preset/chain override values are applied directly |
