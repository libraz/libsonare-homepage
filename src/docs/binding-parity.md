...
| GM program following in a synth bounce | C ABI (`use_gm_programs`) and Python (`auto_select_gm=`); the WASM and Node bounce bindings take a fixed patch per destination | Yes — the bare `--synth` flag |
...
| `Audio` sample access | WASM's `audio.data` is the instance's mutable `Float32Array`; writing into it changes what later instance methods read. Node's `audio.getData()` and Python's `audio.data` return copies, so writing into those arrays does not edit the instance and each access allocates |
