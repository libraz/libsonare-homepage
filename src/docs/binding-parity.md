...
| `Audio` sample access | `audio.getData()` on Node and WASM returns a **copy**, so writing into it does not edit the instance and each call allocates; Python's `audio.data` follows the same read-only contract. Build a new instance from edited samples rather than mutating in place |
