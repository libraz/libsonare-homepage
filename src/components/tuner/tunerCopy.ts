...
export const enCopy = {
  ...
  target: {
    ...
    gsNote:
      'A GS drum kit re-voices the A/B under that kit; you tune the shared Standard base patch and the kit transform is applied on top. A melodic GS variation (CC0) only records the slot address — the A/B renders the capital tone.',
    kitScope: {
      0: '',
      1: 'Room: more shell body and a longer, ambient tail across the whole kit.',
      2: 'Power (Rock): bigger, lower, longer kick / snare / toms.',
      3: 'Electronic: kick / snare / toms become dry decaying sines.',
      4: 'TR-808: the classic decaying-sine kick / snare / toms, brighter cymbals.',
      5: 'Jazz: tighter, higher, softer kick / snare / toms.',
      6: 'Brush: the snare becomes a sustained swish instead of a crack.',
      7: 'Orchestra: long concert bass-drum / timpani rings and longer cymbals.',
      8: 'SFX kit is a set of one-shots in GS — it sounds like the Standard kit here.',
  ...
  macros: {
    ...
    items: {
      ...
      rattle: { label: 'Snare wires', hint: 'No wires → aggressive snare-wire rattle.' },
      shimmer: { label: 'Cymbal wash', hint: 'Static ring → blooming high cymbal wash.' },
      grains: { label: 'Shaker grains', hint: 'A few sparse beads → a dense, smooth shake.' },
...
export const jaCopy: TunerCopy = {
  ...
  target: {
    ...
    gsNote:
      'GS ドラムキットを選ぶと A/B がそのキットの音に切り替わります。編集するのは共有の Standard ベースパッチで、キット変形はその上に適用されます。音程ありの GS バリエーション (CC0) はスロットのアドレスを記録するだけで、A/B はもとの音色を再生します。',
    kitScope: {
      0: '',
      1: 'ルーム: 胴の鳴りが増し、キット全体に余韻のある長めのテールが付きます。',
      2: 'パワー（ロック）: キック／スネア／タムが大きく低く長くなります。',
      3: 'エレクトロニック: キック／スネア／タムがドライな減衰サイン波になります。',
      4: 'TR-808: 定番の減衰サイン波のキック／スネア／タム、シンバルはやや明るく。',
      5: 'ジャズ: キック／スネア／タムが締まって高く柔らかくなります。',
      6: 'ブラシ: スネアがクラックではなく持続するスウィッシュになります。',
      7: 'オーケストラ: コンサートバスドラム／ティンパニの長い響きとシンバルの延び。',
      8: 'SFX キットは GS ではワンショット集で、ここでは Standard キットと同じ音です。',
    } as Record<number, string>,
  },
  chain: {
    exciter: '励振',
  ...
  macros: {
    ...
    items: {
      ...
      rattle: { label: 'スナッピー', hint: 'ワイヤなし → 強く鳴るスナッピーのバズ。' },
      shimmer: { label: 'シンバルの響き', hint: '静的なリング → 咲き広がる高域のウォッシュ。' },
      grains: { label: 'シェイカーの粒', hint: 'まばらな粒 → 密で滑らかなシェイク。' },
    },
  },
  engines: {
    'karplus-strong': {
