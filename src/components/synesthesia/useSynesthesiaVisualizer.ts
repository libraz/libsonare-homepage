...
export function useSynesthesiaVisualizer(props: SynesthesiaVisualizerProps) {
  const { isDark } = useTheme();

  // Theme-aware canvas colors
  const gridStroke = (a: number) =>
    isDark.value ? `rgba(255, 255, 255, ${a})` : `rgba(100, 80, 160, ${a})`;
  const idleCenterFill = computed(() =>
    isDark.value ? 'rgba(15, 18, 28, 0.9)' : 'rgba(240, 238, 248, 0.9)',
  );
  const scanLineColor = computed(() =>
    isDark.value ? 'rgba(255, 255, 255, 0.5)' : 'rgba(100, 80, 160, 0.3)',
  );
  const hubStops = computed(() =>
    isDark.value
      ? ['rgba(20, 25, 35, 0.95)', 'rgba(15, 18, 28, 0.95)', 'rgba(10, 12, 20, 0.9)']
      : ['rgba(255, 255, 255, 0.95)', 'rgba(245, 243, 255, 0.92)', 'rgba(232, 226, 250, 0.88)'],
  );
  ...
  const currentFrame = computed(() => {
    ...
    const compensatedTime = Math.max(
      0,
      Math.min(props.currentTime + LATENCY_COMPENSATION, props.duration),
    );
  ...
  function drawBurstParticles(centerX: number, centerY: number, maxRadius: number) {
    ...
    for (let i = burstParticles.length - 1; i >= 0; i--) {
      ...
      const easeOut = 1 - (1 - progress) ** 2.5;
      ...
      if (p.layer === 2) {
        ...
        for (let t = 0; t < trailLength; t++) {
          ...
          const trailEase = 1 - (1 - trailProgress) ** 2.5;
  ...
  function draw() {
    ...
    if (isActive && props.chromaData && props.rmsData) {
      ...
      const flareIntensity = smoothedRms.value ** 1.8;
      ...
      if (props.bandData) {
        const bandFrame = Math.floor(
          (frame / props.chromaData.nFrames) * props.bandData.low.length,
        );
      ...
      if (melodyIntensity > 0.08) {
        ...
        const melodyGrad = ctx.createLinearGradient(
          centerX + melodyRadius,
          centerY,
          centerX,
          centerY - melodyRadius,
        );
      ...
      const peakIntensity =
        smoothedRms.value > peakThreshold
      ...
      hubGrad.addColorStop(0, hubStops.value[0]);
      hubGrad.addColorStop(0.7, hubStops.value[1]);
      hubGrad.addColorStop(1, hubStops.value[2]);
      ctx.beginPath();
      ...
      for (let i = 0; i < 12; i++) {
        ...
        if (value > 0.02) {
          ...
          const innerStart = maxRadius * 0.5;
          ...
          barGrad.addColorStop(
            1,
            hsl(color.h, color.s, color.l + 10, value * 0.5 + value * 0.4 * noteScale),
          );
          ctx.beginPath();
          ctx.moveTo(x1, y1);
          ctx.lineTo(x2, y2);
