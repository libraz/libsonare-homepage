...
/**
 * Free-reed parity harness: renders a sustained free-reed note through the real
 * libsonare WASM core (offline bounce) and through the TS `FreeReedVoiceCore`
 * port with the SAME deep params, then asserts they are the same instrument over
 * a STEADY analysis window. Free reeds self-sustain, so the RMS envelope is
 * near-flat (envCorr is logged, not asserted); the spectral envelope is the
 * ground-truth metric.
 */
import { beforeAll, describe, expect, it } from 'vitest';
import { defaultFreeReedParams, FreeReedVoiceCore } from '@/tuner/dsp/free-reed-voice';
...
/** Drive the free-reed core directly with a simple inline amp envelope. */
function renderFreeReed(
  params: ReturnType<typeof defaultFreeReedParams>,
  note: number,
  velocity: number,
  frames: number,
): Float32Array {
  const core = new FreeReedVoiceCore();
...
describe('Free-reed parity vs WASM core (engine defaults)', () => {
  it('matches the steady-state spectral envelope', () => {
    const note = 55; // G3 = ~196.0 Hz, mid accordion range
    const velocity = 100;
    const seconds = 1.0;
    const frames = Math.round(SR * seconds);

    const ref = bounceWasm(transparentPatch('free-reed'), note, velocity, seconds);
    const refPeak = Math.max(...Array.from(ref, Math.abs));
    expect(refPeak).toBeGreaterThan(0.005);

    const port = renderFreeReed(defaultFreeReedParams(), note, velocity, frames);
    ...
    console.log(
      `[parity free-reed] specCorr=${specCorr.toFixed(3)} envCorr=${envCorr.toFixed(3)} refPeak=${refPeak.toFixed(3)} portPeak=${portPeak.toFixed(3)}`,
    );
