...
/**
 * Vocal parity harness: renders a sustained sung note through the real
 * libsonare WASM core (offline bounce) and through the TS `VocalVoiceCore` port
 * with the SAME deep params, then asserts they are the same instrument over a
 * STEADY analysis window. The voice self-sustains, so the RMS envelope is
 * near-flat (envCorr is logged, not asserted); the spectral envelope is the
 * ground-truth metric.
 */
import { beforeAll, describe, expect, it } from 'vitest';
import { defaultVocalParams, VocalVoiceCore } from '@/tuner/dsp/vocal-voice';
...
/** Drive the vocal core directly with a simple inline amp envelope. */
function renderVocal(
  params: ReturnType<typeof defaultVocalParams>,
  note: number,
  velocity: number,
  frames: number,
): Float32Array {
  const core = new VocalVoiceCore();
...
describe('Vocal parity vs WASM core (engine defaults)', () => {
  it('matches the steady-state spectral envelope', () => {
    const note = 48; // C3 = ~130.8 Hz, bass-voice range (the vowel tables are sung-bass)
    const velocity = 100;
    const seconds = 1.0;
    const frames = Math.round(SR * seconds);

    const ref = bounceWasm(transparentPatch('vocal'), note, velocity, seconds);
    const refPeak = Math.max(...Array.from(ref, Math.abs));
    expect(refPeak).toBeGreaterThan(0.005);

    const port = renderVocal(defaultVocalParams(), note, velocity, frames);
    ...
    console.log(
      `[parity vocal] specCorr=${specCorr.toFixed(3)} envCorr=${envCorr.toFixed(3)} refPeak=${refPeak.toFixed(3)} portPeak=${portPeak.toFixed(3)}`,
    );
