...
/**
 * Plucked-string parity harness: renders a plucked note through the real
 * libsonare WASM core (offline bounce) and through the TS `PluckedStringVoiceCore`
 * port with the SAME deep params, then asserts they are the same instrument over
 * an early-sustain analysis window (before the string decays too far). The RMS
 * envelope is logged, not asserted; the spectral envelope is the ground-truth
 * metric.
 */
import { beforeAll, describe, expect, it } from 'vitest';
import { defaultPluckedParams, PluckedStringVoiceCore } from '@/tuner/dsp/plucked-string-voice';
...
/** Drive the plucked-string core directly with a simple inline amp envelope. */
function renderPlucked(
  params: ReturnType<typeof defaultPluckedParams>,
  note: number,
  velocity: number,
  frames: number,
): Float32Array {
  const core = new PluckedStringVoiceCore(SR);
  core.start(params, SR, note, velocity, voiceSeed(0, note, 0n));
  const attackInc = 1 / Math.max(1, 0.005 * SR);
  const releaseCoeff = Math.exp(-6.907755279 / Math.max(1, 0.06 * SR));
  const out = new Float32Array(frames);
  let level = 0;
  for (let i = 0; i < frames; ++i) {
    if (level < 1) level = Math.min(1, level + attackInc);
    out[i] = core.render(1) * level;
  }
  // releaseCoeff is defined for symmetry with the wrapper; the note sustains
  // through the whole window so no release stage runs.
  void releaseCoeff;
  return out;
}
...
describe('Plucked-string parity vs WASM core (engine defaults)', () => {
  it('matches the steady-state spectral envelope', () => {
    const note = 52; // E3 = ~164.8 Hz, mid harp/koto range
    const velocity = 100;
    const seconds = 1.0;
    const frames = Math.round(SR * seconds);

    const ref = bounceWasm(transparentPatch('plucked-string'), note, velocity, seconds);
    const refPeak = Math.max(...Array.from(ref, Math.abs));
    expect(refPeak).toBeGreaterThan(0.005);

    const port = renderPlucked(defaultPluckedParams(), note, velocity, frames);
    ...
    console.log(
      `[parity plucked-string] specCorr=${specCorr.toFixed(3)} envCorr=${envCorr.toFixed(3)} refPeak=${refPeak.toFixed(3)} portPeak=${portPeak.toFixed(3)}`,
    );
