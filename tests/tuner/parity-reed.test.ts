...
/**
 * Reed-woodwind parity harness: renders a sustained reed note through the real
 * libsonare WASM core (offline bounce) and through the TS `ReedVoiceCore` port
 * with the SAME deep params, then asserts they are the same instrument over a
 * STEADY analysis window. Reeds self-sustain, so the RMS envelope is near-flat
 * (envCorr is logged, not asserted); the spectral envelope is the ground-truth
 * metric.
 */
import { beforeAll, describe, expect, it } from 'vitest';
import { defaultReedParams, ReedVoiceCore } from '@/tuner/dsp/reed-voice';
...
/** Drive the reed core directly with a simple inline amp envelope. */
function renderReed(
  params: ReturnType<typeof defaultReedParams>,
  note: number,
  velocity: number,
  frames: number,
): Float32Array {
  const core = new ReedVoiceCore(SR);
...
describe('Reed parity vs WASM core (engine defaults)', () => {
  it('matches the steady-state spectral envelope', () => {
    const note = 50; // D3 = ~146.8 Hz, mid clarinet range
    const velocity = 100;
    const seconds = 1.0;
    const frames = Math.round(SR * seconds);

    const ref = bounceWasm(transparentPatch('reed'), note, velocity, seconds);
    const refPeak = Math.max(...Array.from(ref, Math.abs));
    expect(refPeak).toBeGreaterThan(0.005);

    const port = renderReed(defaultReedParams(), note, velocity, frames);
    ...
    console.log(
      `[parity reed] specCorr=${specCorr.toFixed(3)} envCorr=${envCorr.toFixed(3)} refPeak=${refPeak.toFixed(3)} portPeak=${portPeak.toFixed(3)}`,
    );
