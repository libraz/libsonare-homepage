...
/**
 * Parity harness for the air-jet FLUTE core: renders a sustained note through the
 ...
 * spectral envelope over a steady window. The flute SELF-OSCILLATES and sustains,
 * so the decay-envelope metric is logged only, not asserted.
 */
import { beforeAll, describe, expect, it } from 'vitest';
import { defaultFluteParams, type FlutePatchParams, FluteVoiceCore } from '@/tuner/dsp/flute-voice';
...
/** Drive the TS core directly with an inline attack/sustain amp envelope. */
function renderFlute(
  ...
  const core = new FluteVoiceCore(SR);
  core.start(params, SR, note, velocity, voiceSeed(0, note, 0n));
  const out = new Float32Array(frames);
  const attackSamples = Math.max(1, Math.round(0.008 * SR));
  for (let i = 0; i < frames; ++i) {
    const amp = i < attackSamples ? i / attackSamples : 1;
    out[i] = amp * core.render(1);
  }
  return out;
}
...
describe('Flute parity vs WASM core (engine defaults)', () => {
  it('matches spectral envelope over the steady window', () => {
    const note = 72; // C5 ~ 523 Hz
    const seconds = 1.0;
    const frames = Math.round(SR * seconds);

    // Vibrato-free params for both sides so the LFO does not smear the spectrum.
    const params = { ...defaultFluteParams(), vibratoDepth: 0 };
    const ref = bounceWasm(transparentPatch('flute'), note, 100, seconds);
    const port = renderFlute(params, note, 100, frames);

    const m = report('flute', ref, port, 5000);
