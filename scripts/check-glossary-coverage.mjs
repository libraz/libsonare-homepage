...
for (const entry of entries) {
  ...
  if (!entry.title?.en || !entry.title?.ja) failures.push(`${entry.id}: missing title.en/title.ja`)
...
for (const entry of published) {
  ...
  if (fs.existsSync(enPath)) checkPublishedPage(`en ${entry.path}`, enPath)
  if (fs.existsSync(jaPath)) checkPublishedPage(`ja ${entry.path}`, jaPath)
}
...
checkGlossaryIndexes()
checkMasteringSidebar()
...
console.log(`glossary coverage check passed: ${published.length} published, ${todo.length} planned`)

function checkPublishedPage(label, filePath) {
  ...
  checkRelatedLinks(label, content)
  const detailsBody = content.match(/^:{3,}\s+details\s+[^\n]*\n([\s\S]*?)\n:{3,}\s*$/m)?.[1]?.trim()
  if (!detailsBody) {
...
function checkRelatedLinks(label, content) {
...
function checkMasteringSidebar() {
  ...
  for (const entry of published) {
    ...
    if (!config.includes(`link: '${enLink}'`)) failures.push(`en sidebar missing glossary link: ${enLink}`)
    if (!config.includes(`link: '${jaLink}'`)) failures.push(`ja sidebar missing glossary link: ${jaLink}`)
  }
}
